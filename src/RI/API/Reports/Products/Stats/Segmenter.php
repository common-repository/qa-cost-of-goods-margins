...
class Segmenter extends ReportsSegmenter
{
    ...
    protected function get_segments($type, $query_params, $table_name)
    {
        ...
        $product_segmenting_table = $wpdb->prefix . 'qa_cog_order_product_lookup';
        ...
        if ('product' === $this->query_args['segmentby']) {
			...
			LEFT JOIN {$wpdb->prefix}qa_cog_category_lookup ON {$wpdb->term_relationships}.term_taxonomy_id = {$wpdb->prefix}qa_cog_category_lookup.category_id
			";
            $segmenting_where          = " AND {$wpdb->prefix}qa_cog_category_lookup.category_tree_id IS NOT NULL";
            $segmenting_groupby        = "{$wpdb->prefix}qa_cog_category_lookup.category_tree_id";
            ...
            if (isset($this->query_args['categories'])) {
                ...
                $segmenting_where .= " AND {$wpdb->prefix}qa_cog_category_lookup.category_id IN ( $category_ids )";
            }
